    ...
    $stmt->execute([':id' => $idMesa]);
        ...
        if ($mesa) {
            if ($mesa['estado'] === 'libre') {
                ...
                // Buscar el último registro histórico sin hora de desocupación para esta mesa
                ...
                if (!$historico || $historico['idUsuario'] != $idUsuario) {
                    ...
                    header('Location: ./hoenn.php?select=' . $idMesa);
                    exit;
                }
...
try {
    ...
    // ==============================================
    $sql = "SELECT * FROM mesa WHERE idSala = 3";
...
// --- 3. SI SE HA SELECCIONADO UNA MESA ---
$selectedMesa = null;
...
<head>
    <meta charset="utf-8">
    <title>Terraza Hoenn</title>
    <link rel="stylesheet" href="./../../../styles/estilos.css">
    <!-- SweetAlert2 -->
    <script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>
</head>
<body class="body-hoenn">
    ...
    <div class="sala-buttons">
        <a href="./kanto.php" class="btn-link">Kanto</a>
        <a href="./johto.php" class="btn-link">Johto</a>
        <a href="./hoenn.php" class="btn-link" disabled>Hoenn</a>
    </div>
        ...
        <div class="filtro-grupo">
            ...
            <div class="filtro-botones">
                <a href="?filtro_estado=todas&filtro_sillas=<?= $filtro_sillas ?>" 
                   class="filtro-btn <?php if ($filtro_estado === 'todas') { echo 'filtro-activo'; } ?>">
                    Todas (<?= $total_mesas ?>)
                </a>
                <a href="?filtro_estado=ocupadas&filtro_sillas=<?= $filtro_sillas ?>" 
                   class="filtro-btn <?php if ($filtro_estado === 'ocupadas') { echo 'filtro-activo'; } ?>">
                    Ocupadas (<?= $ocupadas ?>)
                </a>
                <a href="?filtro_estado=libres&filtro_sillas=<?= $filtro_sillas ?>" 
                   class="filtro-btn <?php if ($filtro_estado === 'libres') { echo 'filtro-activo'; } ?>">
                    Libres (<?= $libres ?>)
                </a>
            </div>
        </div>
        ...
        <div class="filtro-grupo">
            ...
            <div class="filtro-botones">
                <a href="?filtro_estado=<?= $filtro_estado ?>&filtro_sillas=todas" 
                   class="filtro-btn <?php if ($filtro_sillas === 'todas') { echo 'filtro-activo'; } ?>">
                    Todas
                </a>
                <a href="?filtro_estado=<?= $filtro_estado ?>&filtro_sillas=1" 
                   class="filtro-btn <?php if ($filtro_sillas === '1') { echo 'filtro-activo'; } ?>">
                    1 (<?= $sillas_1 ?>)
                </a>
                <a href="?filtro_estado=<?= $filtro_estado ?>&filtro_sillas=2" 
                   class="filtro-btn <?php if ($filtro_sillas === '2') { echo 'filtro-activo'; } ?>">
                    2 (<?= $sillas_2 ?>)
                </a>
                <a href="?filtro_estado=<?= $filtro_estado ?>&filtro_sillas=3" 
                   class="filtro-btn <?php if ($filtro_sillas === '3') { echo 'filtro-activo'; } ?>">
                    3 (<?= $sillas_3 ?>)
                </a>
                <a href="?filtro_estado=<?= $filtro_estado ?>&filtro_sillas=4" 
                   class="filtro-btn <?php if ($filtro_sillas === '4') { echo 'filtro-activo'; } ?>">
                    4 (<?= $sillas_4 ?>)
                </a>
            </div>
        </div>
    ...
    <div class="info-sala">
        ...
        <?php if (!$selectedMesa): ?>
            ...
            <ul>
                ...
                <br><br><br>
                <a class="btn-toggle" href="../../historialSala.php?idSala=3">Ver historial de sala</a>
            ...
            <?php else: ?>
                ...
                <!-- FORM CON AMBOS FILTROS ACTUALIZADO -->
                <form id="form-cambiar-estado" method="post" action="./hoenn.php?filtro_estado=<?= $filtro_estado ?>&filtro_sillas=<?= $filtro_sillas ?>">
                    <input type="hidden" name="idMesa" value="<?= intval($selectedMesa['idMesa']) ?>">
                    <button type="submit" class="btn-toggle" 
                        <?php if ($selectedMesa['estado'] === 'ocupada' && !$puedeLiberar) { echo 'disabled style="background:#ccc; cursor:not-allowed;"'; } ?>>
                        <?php if ($selectedMesa['estado'] === 'libre') { echo 'Marcar como ocupada'; } else { echo 'Marcar como libre'; } ?>
                    </button>
                </form>
            ...
            <?php else: ?>
                <?php foreach ($mesas as $m): ?>
                    <?php
                        ...
                        $url = "hoenn.php?select=" . intval($m['idMesa']) . 
